document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderPricing(p, hadCoupon) {
        if (els.pSub) els.pSub.textContent = fmtVnd(p.subtotal || 0);
        if (els.pDiscount) els.pDiscount.textContent = fmtVnd(p.discount || 0);
        if (els.pShip) els.pShip.textContent = fmtVnd(p.shipping || 0);
        if (els.pTotal) els.pTotal.textContent = fmtVnd(p.total || 0);
        if (els.dockTotal) els.dockTotal.textContent = fmtVnd(p.total || 0);
        if (els.msg) {
            if (hadCoupon) {
                els.msg.innerHTML = (p.discount || 0) > 0
                    ? `<span class="ok">Đã áp dụng mã giảm giá</span>`
                    : `<span class="err">Mã giảm giá không hợp lệ</span>`;
            } else {
                els.msg.textContent = '';
            }
        }
        ...
        if (dockCoupon) {
            const couponCode = (els.coupon?.value || '').trim();
            dockCoupon.textContent = (p.discount > 0 && couponCode) ? couponCode : 'chưa áp dụng';
        }
    }
    ...
    function updateVoucherNav() {
        ...
        if (els.navPrev) els.navPrev.disabled = x <= 1;
        if (els.navNext) els.navNext.disabled = x >= max - 1;
    }
    ...
    document.querySelectorAll('input[name="pay"]').forEach(r => {
        r.addEventListener('change', updateDockPayChip);
    });
